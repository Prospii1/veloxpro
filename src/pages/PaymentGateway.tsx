...
import { Wallet, Loader2, ArrowLeft, CheckCircle2, AlertTriangle, Bitcoin, CreditCard, ExternalLink, ShieldCheck } from 'lucide-react';
...
export const PaymentGateway: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const [pendingReference, setPendingReference] = useState<string | null>(localStorage.getItem('pending_funding_ref'));
  ...
  const handleFundWallet = async () => {
    ...
    try {
      // Step 1: Initialize Korapay checkout (backend creates the Pending record)
      const resp = await initializeWalletFund(amount as number, user.email || '', user.id);

      if (!resp.success) throw new Error(resp.error || 'Failed to initialize payment');

      const { reference, checkout_url } = resp.data;

      // Save reference for the manual confirmation flow
      localStorage.setItem('pending_funding_ref', reference);
      localStorage.setItem('pending_funding_amount', String(amount));
      setPendingReference(reference);

      if (checkout_url) {
        // Open the test/real checkout in a NEW tab
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      // Step 2: Verify payment (backend updates the Pending record to Completed)
      const verifyResp = await verifyWalletPayment(pendingReference);
      ...
      await supabase.rpc('log_user_activity', {
        ...
        p_description: `User confirmed wallet funding of ${formatPrice(Number(localStorage.getItem('pending_funding_amount')) || 0)} via Korapay.`,
        p_device_info: navigator.userAgent
      });

      // Step 4: Cleanup and show success
      localStorage.removeItem('pending_funding_ref');
      localStorage.removeItem('pending_funding_amount');
      ...
      <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-6 md:p-10 relative overflow-hidden">
        {success ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-slate-400">Your new balance: <span className="font-bold text-emerald-500">{formatPrice(currentBalance)}</span></p>
            <button
              onClick={onBack}
                ...
                <>
                  {pendingReference ? (
                    <div className="space-y-6">
                      <div className="p-6 bg-amber-50 rounded-2xl border border-amber-100">
                        <div className="flex items-center gap-3 text-amber-600 font-bold mb-3">
                          <ExternalLink size={20} /> Payment Initiated
                        </div>
                        <p className="text-sm text-amber-500/80 mb-4">
                          We've opened the Korapay test link in a new tab. Please complete your payment there, then return here to confirm.
                        </p>
                        <button 
                          onClick={() => window.open('https://test-checkout.korapay.com/pay/D0OnAzDUL54CNph', '_blank')}
                      ...
                      <div className="flex flex-col gap-3">
                        <button 
                          ...
                          disabled={loading}
                          className="w-full bg-emerald-500 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-emerald-600 transition-all flex items-center justify-center gap-2"
                        >
                          {loading ? <Loader2 size={20} className="animate-spin" /> : <><ShieldCheck size={20} /> I have paid, Confirm Now</>}
                        </button>
                        <button 
                          onClick={() => {
                            localStorage.removeItem('pending_funding_ref');
                    ...
                    <>
                      ...
                      {error && (
                        <div className="p-4 bg-red-50 text-red-600 rounded-xl text-center font-bold flex items-center justify-center gap-2">
